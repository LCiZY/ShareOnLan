...
public class FileUtils {
    ...
    public static ArrayList<String> getFilesAllName(String path) {
        ...
        long[] fileLastModifyTime = new long[files.length];
        for(int i =0;i<files.length;i++){
            s.add(files[i].getAbsolutePath());
            fileLastModifyTime[i] = files[i].lastModified();
        }

        for(int i=1;i<s.size();i++){
            String temp = s.get(i);
            long ltemp = fileLastModifyTime[i];
            int j;
            for(j=i;j-1>=0;j--){
                if(ltemp>fileLastModifyTime[j-1]){
                    fileLastModifyTime[j] = fileLastModifyTime[j-1];
                    s.set(j,s.get(j-1));
                }else break;
            }
            fileLastModifyTime[j] = ltemp;
            s.set(j,temp);
        }
    ...
    public static boolean deleteAllFileInDir(String path){
        try {
            File file = new File(path);
