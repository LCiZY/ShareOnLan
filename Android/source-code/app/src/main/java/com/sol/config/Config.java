...
public class Config {
    ...
    public static void varInit(Context context) {
        ...
        Log.d(TAG, "配置文件路径:" + context.getExternalFilesDir(null).getPath());
        TODIRRETORY = APPROOTDIR + File.separator;
        RECEIVEFILEDIRETORY = TODIRRETORY + RECEIVEDIROTERYNAME + File.separator;
    }
    ...
    public static void initDuringFirstLaunch() {
        ...
        try {
            if (!file.exists()) {
                ...
                //建立新文件
                Log.d(TAG, TODIRRETORY);
                File dir = new File(TODIRRETORY);
                Log.d(TAG, "创建文件：" + dir.mkdirs());
