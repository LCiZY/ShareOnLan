...
private:
    QPoint mousePressPoint, mouseMoveCurrPoint, windowInitialPoint;  // 移动主窗口所用变量,分别为鼠标点击起始点、鼠标移动后的点、窗口初始位置
    bool isDragging;// 是否处于拖拽状态
    int rightBorder, bottomBorder; // 分别代表鼠标点击可触发移动区域的x和y,鼠标可触发的区域为Point(lefttop)~Point(lefttop)+Point(xy)
};
