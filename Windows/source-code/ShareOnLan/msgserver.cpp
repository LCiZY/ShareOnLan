...
msgServer::msgServer(QObject *, int port) {
  ...
  checkClientAliveTimerID = startTimer(
      (timeOutValue - 4) *
      1000); // timeOutValue是Android端读超时时间readTimeOutValue，PC端必须在超时之前发送心跳包给Android端
  checkIPClientTimerID = startTimer(4000);
}

msgServer::~msgServer() { socket->deleteLater(); }

void msgServer::incomingConnection(int descriptor) {

  if (this->socket != nullptr) {
    log::warn("%s", "已存在其他TCP连接！拒绝连接");
    return;
  }
  pauseAccepting();
...
void msgServer::incomingConnection(QTcpSocket *socket) {
  ...
  if (!this->socket->waitForConnected()) {
    log::info(
        "%s",
        tr("error:%1").arg(this->socket->errorString()).toStdString().c_str());
  }
  connect(socket, SIGNAL(readyRead()), this, SLOT(readMsg()));
  connect(socket, SIGNAL(disconnected()), this, SLOT(socketDisconnect()));
  log::info("%s", (tr("接收到控制连接, socketDescriptor: ") +
                   QString::number(socket->socketDescriptor()) + tr("  ip:") +
                   getIPv4(socket->peerAddress().toIPv4Address()) +
                   tr("  port:") + QString::number(socket->peerPort()))
                      .toStdString()
                      .c_str());
  //发送client改变消息
  emit(clientChange());
...
void msgServer::socketDisconnect() {
  ...
  if (this->socket != nullptr) {
    log::info("%s", (tr("控制连接断开，socket descriptor：") +
                     QString::number(this->socket->socketDescriptor()))
                        .toStdString()
                        .c_str());
  ...
  if (this->socket != nullptr)
    while (this->socket->bytesAvailable() > 0) {
      ...
      if (lastMsg.indexOf(AppContext::FileInfoMsgPreffix) == 0) { //收到文件信息
        log::info(
            "%s",
            (tr("接收到移动端文件信息：") + lastMsg).toStdString().c_str());
        FileInfo *rt = utils::parseFileInfoMsg(lastMsg);
        AppContext::receiveFilesQueue.enqueue(rt);
        send(FILEINFORESPONSE);
      } else if (lastMsg.compare(FILEINFORESPONSE) ==
                 0) { //对方已经收到文件信息了
        emit otherPCReadyReceiveFile();
      } else if (lastMsg.compare(RESPONSE) == 0) { //心跳包
        if (this->msgHeartStack.size())
          this->msgHeartStack.pop();
      } else { //文本信息，写入剪贴板
        log::info(
            "%s",
            (tr("接收到移动端文本消息：") + lastMsg).toStdString().c_str());
        lastMsg = lastMsg.replace(REPLACER, "\r");
        lastMsg = lastMsg.replace(REPLACEN, "\n");
        QClipboard *board = QApplication::clipboard();
        board->setText(lastMsg);
        msgList.append(lastMsg);
        //自动回复: R\n
        send(RESPONSE);
      }
    }
...
bool msgServer::listenOn(int port) {
  ...
  if ((ok = listen(QHostAddress::Any, port))) {
    log::info("%s", QString("消息服务器：监听成功，端口：%1")
                        .arg(QString::number(port))
                        .toStdString()
                        .c_str());
    this->listeningPort = port;
  } else {
    log::error("%s", QString("消息服务器：监听失败，端口：%1")
                         .arg(QString::number(port))
                         .toStdString()
                         .c_str());
  }
...
bool msgServer::ifConnected() { return this->socket != nullptr; }

QString msgServer::getConnection() {
  return ifConnected()
             ? ("连接至" + getIPv4(this->socket->peerAddress().toIPv4Address()))
             : "无连接";
}

const QTcpSocket *msgServer::getSocket() { return this->socket; }
  ...
  if (checkClientAliveTimerID == e->timerId()) {
    ...
    if (this->socket != nullptr) {
      ...
      if (this->socket->state() != 3) {
        ...
        log::error("检测到连接异常(wrong state)，自动断开");
      }
      send(RESPONSE);
      this->socket->waitForBytesWritten();
      msgHeartStack.push(0);
      if (msgHeartStack.size() > 2) {
        this->socket->close();
        errorTimes = 0;
        msgHeartStack.clear();
        socketDisconnect();
        log::error("检测到连接异常(timeout)，自动断开");
      }
    } else if (AppContext::ipList.size() != 0) {
      //如果还没连接：发送ip、端口号、连接密钥的信息
      for (int i = 0; i < AppContext::ipList.size(); i++) {
        netInfoStr = AppContext::ipList.at(i) + tr(" ") +
                     setting->get(UserSetting::Item::PORT) + tr(" ");
        QHostAddress host = QHostAddress(AppContext::brocastList.at(i));
        QString info =
            encrypt(netInfoStr + setting->get(UserSetting::Item::SECRET));
        std::string strTemp = info.toStdString();
        const char *temp = strTemp.c_str();
        // qDebug()<<"发送UDP报文："<<netInfoStr+setting->get(UserSetting::Item::SECRET)+"
        // 加密后："<<temp;
        this->udpSocket->writeDatagram(temp, strlen(temp), host,
                                       DEFAULT_IP_PORT_UDP_PORT);
    ...
    if (AppContext::ipListCurr.size() != AppContext::ipList.size()) {
      ...
      for (int i = 0; i < AppContext::ipListCurr.size(); i++) {
        ...
        for (int j = 0; j < AppContext::ipList.size(); j++) {
          if (AppContext::ipListCurr.at(i) == AppContext::ipList.at(j)) {
            flag_1 = true;
            break;
          }
        }
...
QString msgServer::encrypt(QString msg) {
  ...
  for (int i = 0; i <= j; i++) {
    temp[i] = (char)((int)temp[i] + pow(-1, i) * factor[i % 3] +
                     (i * mc) / temp.length());
  }
  ...
  result.prepend(QChar(c1));
  result.append(QChar(c2));
  return result;
}
...
void msgServer::getLanBrocastAddress() {
  AppContext::ipList.clear();
  AppContext::brocastList.clear();
  AppContext::networkcardList.clear();
  ...
  for (int i = 0; i < interfaceList.count(); i++) {
    ...
    if (networkCardName.contains("Loopback"))
      continue;
    ...
    for (int j = 0; j < entryList.count(); j++) {
      ...
      log::info("%s", (tr("ip：") + ip + tr("  掩码：") + mask +
                       "  广播地址：" + bcast)
                          .toStdString()
                          .c_str());
    }
