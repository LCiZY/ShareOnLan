    ...
    QObject(nullptr)
{
    ...
    //check配置
    //如果端口无效，使用默认端口
    if(!isPortValid()) { set(UserSetting::Item::PORT, defaultConfigurations[getItemKey(UserSetting::Item::PORT)]); }
...
void UserSetting::configInit(){
    ...
    set(UserSetting::Item::FILE_RECEIVE_LOCATION, defaultConfigurations[getItemKey(UserSetting::Item::FILE_RECEIVE_LOCATION)]);
    ...
    if(configFile.open(QIODevice::ReadOnly)){
        ...
        while (lineLength!=-1) {
            ...
            if(splits.size()==2){ //如果配置正确，那么splits的大小应该为2,使用配置文件里的配置
                ...
                qDebug("用户配置 %s: %s", splits.at(0).toStdString().c_str(), splits.at(1).toStdString().c_str());
...
bool UserSetting::writeToConfigFile(QString configurationItem,QString configuration){
    //
    QFile configFile(configFilePath);
...
bool UserSetting::readDefaultConfigFromDefaultConfigFile(){
    ...
    if(configFile.open(QIODevice::ReadOnly)){
        ...
        //文件默认存放位置：动态获取。
        defaultConfigurations[getItemKey(UserSetting::Item::FILE_RECEIVE_LOCATION)] =
