#include "sharing.h"
#include "ui_sharing.h"

#include<QDebug>

sharing::sharing(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::sharing)
{
    ui->setupUi(this);

    this->setWindowTitle(tr("Sharing"));
...
void sharing::on_activatedSysTrayIcon(QSystemTrayIcon::ActivationReason reason)
{
    switch(reason){
...
//监听的端口号有变更
void  sharing::portChange(QString newPort){
...
void sharing::secretChange(QString newSecret){
...
void  sharing::slot_checkBox_ifhidewhenlaunch(bool checked){
    conf->setConfig("ifhidewhenlaunch",checked?"true":"false");
}


void sharing::changeFileReceiveLocation(){
...
void sharing::openFileLocationFolder(){
      QString currDir = ui->lineEdit_fileReceiveLocation->text().compare("")==0?QStandardPaths::writableLocation(QStandardPaths::DesktopLocation):ui->lineEdit_fileReceiveLocation->text();
...
void sharing::on_showMainAction(){
    this->showMaximized();
    this->move(QApplication::desktop()->availableGeometry().width()-rect().width()/2,QApplication::desktop()->availableGeometry().height()-rect().height()/2);
}
...
//断开所有连接
void sharing::on_clearConnection(){
    server->closeSocket();
    fileserver->closeSocket();
}

//重启server：重新监听端口并resume连接并重新检测网络信息
void sharing::on_restartServer(){
...
void sharing::slot_checkBox_autoLaunch(bool flag){
    conf->setConfig("ifautoStartup",flag?"true":"false");
    conf->setAutomaticStartup(flag);
}

void sharing::showMini(){
    this->showMinimized();
}

void sharing::hideWindow(){
    this->hide();
}

void sharing::on_exitAppAction(){
    this->close();
}

void  sharing::showProgressUI(){
...
void  sharing::setProgressInfo(int fileSize, QString fileName){
    if(this->progressui==nullptr) return;
    progressui->setCurrTaskInfo(fileSize,fileName);
}

void sharing::progressUIChange(qint64 pg){
    if(this->progressui==nullptr) return;
    this->progressui->changeUI(pg);
}

void sharing::progressUIDestroy(){
    if(this->progressui==nullptr) return;
...
void sharing::mouseMoveEvent(QMouseEvent *event)
{
    if(moveFlag)
    {
...
void sharing::mousePressEvent(QMouseEvent *event){
...
void sharing::mouseReleaseEvent(QMouseEvent *)
{
    moveFlag=false;
...
void sharing::closeEvent(QCloseEvent* event){
...
bool sharing::detectSingleInstance(){
...
void sharing::newLocalSocketConnection(){
